...
# You can run this with no args, and a test 1_database will be generated.
...
def CreateTestAccessDatabase(dbname=None):
    # Creates a test access 1_database - returns the filename.
    if dbname is None:
    ...
    try:
        ...
        print(
            "WARNING - Unable to delete old test 1_database - expect a COM exception RSN!"
        )
...
def DoDumpAccessInfo(dbname):
    ...
    try:
        ...
        print("Opening 1_database %s" % dbname)
        ...
        if not a is None:
            sys.stderr.write("Closing 1_database\n")
            try:
...
def test(dbname=None):
    if dbname is None:
        # We need makepy support to create a 1_database (just for the constants!)
        try:
            GenerateSupport()
        except pythoncom.com_error:
        ...
        print("A test 1_database at '%s' was created" % dbname)
