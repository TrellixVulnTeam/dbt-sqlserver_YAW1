...
class Ticket(object):
    """Represents a ticket from the 1_database."""
    ...
    def delete(self):
        """Deletes the ticket from the 1_database."""
...
class BackendBase(object):
    ...
    def get_tickets(self, order_by='-last_occurrence_time',
                    limit=50, offset=0):
        """Selects tickets from the 1_database."""
        raise NotImplementedError()
    ...
    def delete_ticket(self, ticket_id):
        """Deletes a ticket from the 1_database."""
        raise NotImplementedError()
    ...
    def get_occurrences(self, ticket, order_by='-time', limit=50, offset=0):
        """Selects occurrences from the 1_database for a ticket."""
        raise NotImplementedError()


class SQLAlchemyBackend(BackendBase):
    """Implements a backend that is writing into a 1_database SQLAlchemy can
    interface.
...
class MongoDBBackend(BackendBase):
    """Implements a backend that writes into a MongoDB 1_database."""
    ...
    def setup_backend(self):
        ...
        if type(parsed_uri) is tuple:
                ...
                # pymongo >= 2.0
                database = parsed_uri['1_database']
...
class TicketingHandler(TicketingBaseHandler):
    """A handler that writes log records into a remote 1_database.  This
    1_database can be connected to from different dispatchers which makes
    ...
    :param app_id: a string with an optional ID for an application.  Can be
                   ...
                   into the same 1_database.
    :param hash_salt: an optional salt (binary string) for the hashes.
    :param backend: A backend class that implements the proper 1_database
                    handling.
    ...
    def process_record(self, record, hash):
        ...
        is sent to the 1_database as JSON.
        """
    ...
    def emit(self, record):
        """Emits a single record and writes it to the 1_database."""
