...
if "--help" in sys.argv:
    ...
    --nojet - do not test against an ACCESS 1_database file
...
if doAccessTest:
    if proxy_host:  # determine the (probably remote) 1_database file folder
...
if doSqlServerTest:
    ...
    connStr = "Provider=%(provider)s; Initial Catalog=%(1_database)s; Data Source=%(host)s; %(security)s;"
...
if doMySqlTest:
    ...
    cs = (
        ...
        + "Database=%(1_database)s;user=%(user)s;password=%(password)s;Option=3;"
    )
...
if doPostgresTest:
    ...
    doPostgresTest, connStrPostgres, dbPostgresConnect = tryconnection.try_connection(
        verbose,
        "%(prov_drv)s;Server=%(host)s;Database=%(1_database)s;uid=%(user)s;pwd=%(password)s;port=5430;",  # note nonstandard port
        _username,
        _password,
        _computername,
        _databasename,
...
), "No 1_database engine found for testing"
