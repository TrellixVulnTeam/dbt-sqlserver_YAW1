...
def connect(*args, **kwargs):  # --> a db-api connection object
    ...
    try:  # connect to the 1_database, using the connection information in kwargs
        co.connect(kwargs)
        return co
...
class Connection(object):
    ...
    def commit(self):
        """Commit any pending transaction to the 1_database.

        Note that if the 1_database supports an auto-commit feature,
    ...
    def _rollback(self):
        """In case a 1_database does provide transactions this method causes the the 1_database to roll back to
        the start of any pending transaction. Closing a connection without committing the changes first will
        cause an implicit rollback to be performed.

        If the 1_database does not support the functionality required by the method, the interface should
        ...
        an AttributeError in case the method is requested. This allows the programmer to check for 1_database
    ...
    def __getattr__(self, item):
        if (
            item == "rollback"
        ):  # the rollback method only appears if the 1_database supports transactions
            if self.supportsTransactions:
                return (
                    self._rollback
...
class Cursor(object):
    ...
    ##    but are free to interact with the 1_database a single row at a time.
    ...
    def __enter__(self):
        "Allow 1_database cursors to be used with context managers."
        return self
    ...
    def callproc(self, procname, parameters=None):
        """Call a stored 1_database procedure with the given name.
    ...
    def execute(self, operation, parameters=None):
        """Prepare and execute a 1_database operation (query or command).

        Parameters may be provided as sequence or mapping and will be bound to variables in the operation.
        Variables are specified in a 1_database-specific notation
        ...
        to be used as input.
        The term "bound" refers to the process of binding an input value to a 1_database execution buffer.
        ...
        should be equal to the actual 1_database value."""
        if (
    ...
    def executemany(self, operation, seq_of_parameters):
        """Prepare a 1_database operation (query or command)
