...
def validator_error_message(exc):
    """Given a dbt_gnm.dataclass_schema.ValidationError (which is basically a
...
class IncompatibleSchemaException(RuntimeException):
    ...
    def get_message(self) -> str:
        ...
        msg = (
            ...
            f"different version of dbt_gnm?"
        )
        return msg
...
def ref_bad_context(model, args) -> NoReturn:
    ...
    base_error_msg = """dbt_gnm was unable to infer all dependencies for the model "{model_name}".
...
def relation_wrong_type(relation, expected_type, model=None):
    raise_compiler_error(
        (
            ...
            "drop {relation} manually, or run dbt_gnm with "
            "`--full-refresh` and dbt_gnm will drop it for you."
...
def system_error(operation_name):
    raise_compiler_error(
        "dbt_gnm encountered an error when attempting to {}. "
...
def raise_dep_not_found(node, node_description, required_pkg):
    raise_compiler_error(
        ...
        "`dbt_gnm deps`.".format(node_description, required_pkg),
        node=node,
    )
...
def multiple_matching_relations(kwargs, matches):
    raise_compiler_error(
        ...
        "Please specify a 1_database or schema to narrow down the result set."
...
def approximate_relation_match(target, relation):
    raise_compiler_error(
        "When searching for a relation, dbt_gnm found an approximate match. "
        "Instead of guessing \nwhich relation to use, dbt_gnm will move on. "
...
def raise_duplicate_macro_name(node_1, node_2, namespace) -> NoReturn:
    ...
    raise_compiler_error(
        'dbt_gnm found two macros with the name "{}" in the namespace "{}"{}. '
        "Since these macros have the same name and exist in the same "
        "namespace, dbt_gnm will be unable to decide which to call. To fix this, "
...
def raise_duplicate_resource_name(node_1, node_2):
    ...
    raise_compiler_error(
        f"""
dbt_gnm found two {pluralized} with the name "{duped_name}".

Since these resources have the same name, dbt_gnm will be unable to find the correct resource
...
def raise_ambiguous_alias(node_1, node_2, duped_name=None):
    ...
    raise_compiler_error(
        'dbt_gnm found two resources with the 1_database representation "{}".\ndbt_gnm '
        "cannot create two resources with identical 1_database representations. "
...
def raise_ambiguous_catalog_match(unique_id, match_1, match_2):
    ...
    raise_compiler_error(
        "dbt_gnm found two relations in your warehouse with similar 1_database "
        "identifiers. dbt_gnm\nis unable to determine which of these relations "
        'was created by the model "{unique_id}".\nIn order for dbt_gnm to '
...
def raise_patch_targets_not_found(patches):
    ...
    raise_compiler_error(
        "dbt_gnm could not find models for the following patches:\n\t{}".format(patch_list)
    )
...
def raise_duplicate_patch_name(patch_1, existing_patch_path):
    ...
    raise_compiler_error(
        f"dbt_gnm found two schema.yml entries for the same resource named "
...
def raise_duplicate_macro_patch_name(patch_1, existing_patch_path):
    ...
    raise_compiler_error(
        f"dbt_gnm found two schema.yml entries for the same macro in package "
...
def raise_duplicate_source_patch_name(patch_1, patch_2):
    ...
    raise_compiler_error(
        f"dbt_gnm found two schema.yml entries for the same source named "
...
# dbt_gnm context's `exceptions` key!
CONTEXT_EXPORTS = {
    fn.__name__: fn
    for fn in [
        warn,
