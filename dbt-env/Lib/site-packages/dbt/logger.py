...
logger = logbook.Logger("dbt_gnm")
# provide this for the cache, disabled by default
CACHE_LOGGER = logbook.Logger("dbt_gnm.cache")
...
class DelayedFileHandler(logbook.RotatingFileHandler, FormatterMixin):
    ...
    def set_path(self, log_dir):
        ...
        writing to a file (for `dbt_gnm debug`).
        """
        if self.disabled:
            return
        ...
        log_path = os.path.join(log_dir, "dbt_gnm.log.legacy")  # TODO hack for now
...
class LogManager(logbook.NestedSetup):
    def __init__(self, stdout=sys.stdout, stderr=sys.stderr):
        ...
        self._relevel_processor = Relevel(allowed=["dbt_gnm", "werkzeug"])
        ...
        # support entering dbt_gnm execution via handle_and_check.
        self._stack_depth = 0
        super().__init__(
            [
                self._null_handler,
    ...
    # this is used by `dbt_gnm ls` to allow piping stdout to jq, etc
...
class ListLogHandler(LogMessageHandler):
    ...
    def should_handle(self, record):
        """Only ever emit dbt_gnm-sourced log messages to the ListHandler."""
        if _root_channel(record) != "dbt_gnm":
            return False
