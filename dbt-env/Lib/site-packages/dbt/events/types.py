...
# | T    | Test only           |
#
# The basic idea is that event codes roughly translate to the natural order of running a dbt_gnm task
...
class MainReportVersion(InfoLevel):
    ...
    def message(self):
        return f"Running with dbt_gnm{self.v}"
...
class MainReportArgs(DebugLevel):
    ...
    def message(self):
        return f"running dbt_gnm with arguments {str(self.args)}"
...
class CacheMiss(DebugLevel):
    ...
    def message(self) -> str:
        return (
            ...
            '"{self.1_database}.{self.schema}", this is inefficient'
        )
...
class ListRelations(DebugLevel):
    ...
    def message(self) -> str:
        return f"with 1_database={self.database}, schema={self.schema}, relations={self.relations}"
...
# see: core/dbt_gnm/adapters/cache.py _add_link vs add_link
@dataclass
class UncachedRelation(DebugLevel, Cache):
    ...
    def message(self) -> str:
        return (
            ...
            "but {self.ref_key.1_database}.{self.ref_key.schema}"
...
class PartialParsingVersionMismatch(InfoLevel):
    ...
    def message(self) -> str:
        return (
            "Unable to do partial parsing because of a dbt_gnm version mismatch. "
...
For more information on configuring profiles, please consult the dbt_gnm docs:
...
class InternalExceptionOnRun(DebugLevel):
    ...
    def message(self) -> str:
        ...
        INTERNAL_ERROR_STRING = """This is an error in dbt_gnm. Please try again. If \
...
class DepsNotifyUpdatesAvailable(InfoLevel):
    ...
    def message(self) -> str:
        return "Updates available for packages: {} \
                \nUpdate your versions in packages.yml, then run dbt_gnm deps".format(
            self.packages
        )
...
class WriteCatalogFailure(ErrorLevel):
    ...
    def message(self) -> str:
        return (
            f"dbt_gnm encountered {self.num_exceptions} failure{(self.num_exceptions != 1) * 's'} "
...
class ConfigFolderDirectory(InfoLevel):
    ...
    def message(self) -> str:
        return f"Creating dbt_gnm configuration folder at {self.dir}"
...
class ProfileWrittenWithTargetTemplateYAML(InfoLevel):
    ...
    def message(self) -> str:
        return (
            ...
            "profile_template.yml and your supplied values. Run 'dbt_gnm debug' to "
