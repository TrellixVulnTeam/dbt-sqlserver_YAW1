...
def _get_core_msg_lines(installed, latest) -> Tuple[List[List[str]], str]:
    ...
    if latest is None:
        update_info = (
            "  The latest version of dbt_gnm-core could not be determined!\n"
    ...
    if installed > latest:
        ...
        update_info = (
            "  Your version of dbt_gnm-core is out of date!\n"
...
def _get_plugins_msg(installed: dbt.semver.VersionSpecifier) -> str:
    ...
    if display_update_msg:
        update_msg = (
            "  At least one plugin is out of date or incompatible with dbt_gnm-core.\n"
            "  You can find instructions for upgrading here:\n"
            "  https://docs.getdbt.com/docs/installation"
        )
...
def _get_dbt_plugins_info() -> Iterator[Tuple[str, str]]:
    for plugin_name in _get_adapter_plugin_names():
        if plugin_name == "core":
            continue
        try:
            mod = importlib.import_module(f"dbt_gnm.adapters.{plugin_name}.__version__")
        except ImportError:
            # not an adapter
            continue
...
def _get_adapter_plugin_names() -> Iterator[str]:
    spec = importlib.util.find_spec("dbt_gnm.adapters")
    # If None, then nothing provides an importable 'dbt_gnm.adapters', so we will
    ...
    for adapters_path in spec.submodule_search_locations:
        ...
        for version_path in glob.glob(version_glob):
            # the path is like .../dbt_gnm/adapters/{plugin_name}/__version__.py
