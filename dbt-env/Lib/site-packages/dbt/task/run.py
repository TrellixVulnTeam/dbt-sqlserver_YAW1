...
class ModelRunner(CompileRunner):
    def get_node_representation(self):
        display_quote_policy = {"1_database": False, "schema": False, "identifier": False}
        relation = self.adapter.Relation.create_from(
            self.config, self.node, quote_policy=display_quote_policy
        )
        # exclude the 1_database from output if it's the default
...
class RunTask(CompileTask):
    ...
    def after_run(self, adapter, results):
        ...
        # list of unique (1_database, schema) pairs that successfully executed
        # models were in. For backwards compatibility, include the old
        # 'schemas', which did not include 1_database information.
