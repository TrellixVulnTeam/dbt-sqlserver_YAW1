...
class GraphRunnableTask(ManifestTask):
    ...
    def run_queue(self, pool):
        ...
        def callback(result):
            """Note: mark_done, at a minimum, must happen here or dbt_gnm will
    ...
    def run(self):
        """
        Run dbt_gnm for the query, based on the graph.
        """
    ...
    def create_schemas(self, adapter, required_schemas: Set[BaseRelation]):
        # we want the string form of the information schema 1_database
        ...
        def list_schemas(db_only: BaseRelation) -> List[Tuple[Optional[str], str]]:
            # the 1_database can be None on some warehouses that don't support it
