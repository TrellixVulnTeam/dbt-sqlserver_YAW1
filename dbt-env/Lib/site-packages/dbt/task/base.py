...
class BaseTask(metaclass=ABCMeta):
    ...
    def from_args(cls, args):
        try:
            ...
            raise dbt.exceptions.RuntimeException("Could not run dbt_gnm") from exc
...
def get_nearest_project_dir(args):
    ...
    if args.project_dir:
        ...
        if os.path.exists(project_file):
            ...
            raise dbt.exceptions.RuntimeException(
                "fatal: Invalid --project-dir flag. Not a dbt_gnm project. "
    ...
    raise dbt.exceptions.RuntimeException(
        "fatal: Not a dbt_gnm project (or any of the parent directories). "
        "Missing dbt_project.yml file"
    )
...
class ExecutionContext:
    """During execution and error handling, dbt_gnm makes use of mutable state:
    ...
    def on_skip(self):
        ...
        if not self.node.is_ephemeral_model:
            ...
            if self._skip_caused_by_ephemeral_failure():
                ...
                # set an error so dbt_gnm will exit with an error code
                error_message = (
