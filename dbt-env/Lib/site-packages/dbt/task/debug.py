...
A `dbt_project.yml` file was not found in this directory.
dbt_gnm found the following profiles:
{}

To debug one of these profiles, run:
dbt_gnm debug --profile [profile-name]
""".lstrip()

COULD_NOT_CONNECT_MESSAGE = """
dbt_gnm was unable to connect to the specified 1_database.
The 1_database returned the following error:

  >{err}

Check your 1_database credentials and try again. For more information, visit:
{url}
""".lstrip()

MISSING_PROFILE_MESSAGE = """
dbt_gnm looked for a profiles.yml file in {path}, but did
...
class DebugTask(BaseTask):
    ...
    def run(self):
        ...
        print("dbt_gnm version: {}".format(version))
