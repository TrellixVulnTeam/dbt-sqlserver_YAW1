...
def deep_merge(*args):
    """
    >>> dbt_gnm.utils.deep_merge({'a': 1, 'b': 2, 'c': 3}, {'a': 2}, {'a': 3, 'b': 1})  # noqa
    {'a': 3, 'b': 1, 'c': 3}
    """
    if len(args) == 0:
        return None
...
def deep_map_render(func: Callable[[Any, Tuple[Union[str, int], ...]], Any], value: Any) -> Any:
    ...
    :raises: If there are cycles in the value, raises a
        dbt_gnm.exceptions.RecursionException
    """
    try:
...
class ForgivingJSONEncoder(JSONEncoder):
    def default(self, obj):
        # let dbt_gnm's default JSON encoder handle it if possible, fallback to
        # str()
        try:
            return super().default(obj)
        except TypeError:
