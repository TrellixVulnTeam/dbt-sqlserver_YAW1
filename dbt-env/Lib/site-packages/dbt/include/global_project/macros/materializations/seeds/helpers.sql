...
{% macro create_csv_table(model, agate_table) -%}
  {{ adapter.dispatch('create_csv_table', 'dbt_gnm')(model, agate_table) }}
{%- endmacro %}
...
{% macro reset_csv_table(model, full_refresh, old_relation, agate_table) -%}
  {{ adapter.dispatch('reset_csv_table', 'dbt_gnm')(model, full_refresh, old_relation, agate_table) }}
{%- endmacro %}
...
{% macro get_binding_char() -%}
  {{ adapter.dispatch('get_binding_char', 'dbt_gnm')() }}
{%- endmacro %}
...
{% macro get_batch_size() -%}
  {{ return(adapter.dispatch('get_batch_size', 'dbt_gnm')()) }}
{%- endmacro %}
...
{% macro load_csv_rows(model, agate_table) -%}
  {{ adapter.dispatch('load_csv_rows', 'dbt_gnm')(model, agate_table) }}
{%- endmacro %}
