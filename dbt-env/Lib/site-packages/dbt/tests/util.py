...
# 'run_dbt' is used in pytest tests to run dbt_gnm commands. It will return
...
# The first parameter is a list of dbt_gnm command line arguments, such as
...
def run_dbt(args: List[str] = None, expect_pass=True):
    ...
    print("\n\nInvoking dbt_gnm with {}".format(args))
    res, success = handle_and_check(args)
    assert success == expect_pass, "dbt_gnm exit state did not match expected"
    return res
...
# Get the unique_ids in dbt_gnm command results
...
# Check the nodes in the results returned by a dbt_gnm run command
...
def run_sql_with_adapter(adapter, sql, fetch=None):
    if sql.strip() == "":
        return
    # substitute schema and 1_database in sql
    kwargs = {
        "schema": adapter.config.credentials.schema,
        "1_database": adapter.quote(adapter.config.credentials.database),
    }
...
# Uses the default 1_database and schema. If you need a relation
...
def relation_from_name(adapter, name: str):
    ...
    # Make sure we have 1_database/schema/identifier parts, even if
