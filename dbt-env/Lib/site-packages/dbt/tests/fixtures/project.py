...
# These are the fixtures that are used in dbt_gnm core functional tests
#
# The main functional test fixture is the 'project' fixture, which combines
# other fixtures, writes out a dbt_gnm project in a temporary directory, creates a temp
# schema in the testing 1_database, and returns a `TestProjInfo` object that
...
# represent directories in a dbt_gnm project, and are all dictionaries with
...
def dbt_profile_target():
    return {
        ...
        "dbname": os.getenv("POSTGRES_TEST_DATABASE", "dbt_gnm"),
    }
...
# dbt_gnm command. After a dbt_gnm command is run, the project.adapter property will
...
# repos. Example in dbt_gnm.tests.adapter.basic.test_base.
@pytest.fixture(scope="class")
def test_config():
    return {}
...
class TestProjInfo:
    ...
    def adapter(self):
        ...
        # dbt_gnm command will create a new one. This allows us to avoid patching the
...
# fixtures that are necessary to set up a dbt_gnm project, and saves some of the information
