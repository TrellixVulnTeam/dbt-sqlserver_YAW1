...
class DBTVersion(argparse.Action):
    ...
    except it just calls dbt_gnm.version.get_version_information().
    """
...
def _build_build_subparser(subparsers, base_subparser):
    ...
    sub.add_argument(
        ...
        action="store_true",
        help="""
        Store test results (failing rows) in the 1_database
        """,
    )
    sub.add_argument(
        "--indirect-selection",
...
def _build_debug_subparser(subparsers, base_subparser):
    sub = subparsers.add_parser(
        ...
        parents=[base_subparser],
        help="""
        Show some helpful information about dbt_gnm for debugging.
...
def _build_run_subparser(subparsers, base_subparser):
    run_sub = subparsers.add_parser(
        ...
        parents=[base_subparser],
        help="""
        Compile SQL and execute against the current target 1_database.
        """,
    )
...
def _build_docs_generate_subparser(subparsers, base_subparser):
    ...
    generate_sub.add_argument(
        ...
        dest="compile",
        help="""
        Do not run "dbt_gnm compile" as part of docs generation
        """,
    )
    return generate_sub
...
def _add_table_mutability_arguments(*subparsers):
    for sub in subparsers:
        sub.add_argument(
            ...
            action="store_true",
            help="""
            If specified, dbt_gnm will drop incremental models and
...
def _add_version_check(sub):
    sub.add_argument(
        ...
        default=None,
        help="""
        If set, skip ensuring dbt_gnm's version matches the one specified in
        the dbt_project.yml file ('require-dbt_gnm-version')
        """,
    )
...
def _build_test_subparser(subparsers, base_subparser):
    sub = subparsers.add_parser(
        ...
        parents=[base_subparser],
        help="""
        Runs tests on data in deployed models. Run this after `dbt_gnm run`
        """,
    )
    sub.add_argument(
        "-x",
...
def _build_run_operation_subparser(subparsers, base_subparser):
    ...
    sub.add_argument(
        "macro",
        help="""
        Specify the macro to invoke. dbt_gnm will call this macro with the supplied
...
def parse_args(args, cls=DBTArgumentParser):
    ...
    p.add_argument(
        ...
        type=str,
        help="""
        When this option is passed, dbt_gnm will output low-level timing stats to
    ...
    p.add_argument(
        ...
        default=None,
        help="""
        Display debug logging during dbt_gnm execution. Useful for debugging and
        making bug reports.
        """,
    )
    ...
    p.add_argument(
        ...
        default=None,
        help="""
        If dbt_gnm would normally warn, instead raise an exception. Examples
    ...
    # if set, run dbt_gnm in single-threaded mode: thread count is ignored, and
    # calls go through `map` instead of the thread pool. This is useful for
    # getting performance information about aspects of dbt_gnm that normally run in
    ...
    p.add_argument(
        ...
        help="""
        Do not send anonymous usage stat to dbt_gnm Labs
        """,
    )
    ...
    schema_cache_flag.add_argument(
        ...
        help="""
        Pre cache 1_database objects relevant to selected resource only.
        """,
    )
    schema_cache_flag.add_argument(
        ...
        dest="cache_selected_only",
        help="""
        Pre cache all 1_database objects related to the project.
        """,
    )
