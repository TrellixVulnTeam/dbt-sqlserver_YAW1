...
This version of dbt_gnm is not supported with the '{package}' package.
  Installed version of dbt_gnm: {installed}
  Required version of dbt_gnm for '{package}': {version_spec}
Check for a different version of the '{package}' package, or run dbt_gnm again with \
--no-version-check
"""
...
package.
  Required versions of dbt_gnm for '{package}': {version_spec}
Check for a different version of the '{package}' package, or run dbt_gnm again with \
--no-version-check
"""
...
def validate_version(dbt_version: List[VersionSpecifier], project_name: str):
    """Ensure this package works with the installed version of dbt_gnm."""
...
def _get_required_version(
    ...
    required = project_dict.get("require-dbt_gnm-version")
...
class PartialProject(RenderComponents):
    ...
    verify_version: bool = field(
        metadata=dict(description=("If True, verify the dbt_gnm version matches the required version"))
    )
...
class Project:
        ...
        result = deepcopy(
            {
                ...
                "require-dbt_gnm-version": [v.to_version_string() for v in self.dbt_version],
    ...
    def get_default_selector_name(self) -> Union[str, None]:
        """This function fetch the default selector to use on `dbt_gnm run` (if any)
