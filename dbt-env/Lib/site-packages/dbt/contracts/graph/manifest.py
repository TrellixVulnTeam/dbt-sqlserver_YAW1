...
class ManifestMetadata(BaseArtifactMetadata):
    ...
    send_anonymous_usage_stats: Optional[bool] = field(
        default=None,
        metadata=dict(
            description=("Whether dbt_gnm is configured to send anonymous usage statistics")
        ),
    )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if equal:
            raise_compiler_error(
                ...
                "{}). dbt_gnm cannot resolve this ambiguity".format(
    ...
    def find_generate_macro_by_name(
        ...
        imported packages.
            - if there is a `generate_{component}_name` macro in the root
              project, return it
            - return the `generate_{component}_name` macro from the 'dbt_gnm'
              internal project
        """
...
class Manifest(MacroMethods, DataClassMessagePackMixin, dbtClassMixin):
    ...
    # Called in dbt_gnm.compilation.Linker.write_graph and
    # dbt_gnm.graph.queue.get and ._include_in_cost
    ...
    # Called by dbt_gnm.parser.manifest._resolve_refs_for_exposure
    # and dbt_gnm.parser.manifest._process_refs_for_node
    def resolve_ref(
        self,
    ...
    # Called by dbt_gnm.parser.manifest._resolve_sources_for_exposure
    # and dbt_gnm.parser.manifest._process_source_for_node
    def resolve_source(
        self,
    ...
    def add_macro(self, source_file: SourceFile, macro: ParsedMacro):
        if macro.unique_id in self.macros:
            ...
            msg = line_wrap_message(
                f"""\
                dbt_gnm found two macros named "{macro.name}" in the project
                "{macro.package_name}".
...
class WritableManifest(ArtifactMixin):
    nodes: Mapping[UniqueID, ManifestNode] = field(
        metadata=dict(description=("The nodes defined in the dbt_gnm project and its dependencies"))
    )
    sources: Mapping[UniqueID, ParsedSourceDefinition] = field(
        metadata=dict(description=("The sources defined in the dbt_gnm project and its dependencies"))
    )
    macros: Mapping[UniqueID, ParsedMacro] = field(
        metadata=dict(description=("The macros defined in the dbt_gnm project and its dependencies"))
    )
    docs: Mapping[UniqueID, ParsedDocumentation] = field(
        metadata=dict(description=("The docs defined in the dbt_gnm project and its dependencies"))
    )
    exposures: Mapping[UniqueID, ParsedExposure] = field(
        metadata=dict(
            description=("The exposures defined in the dbt_gnm project and its dependencies")
        )
    )
    metrics: Mapping[UniqueID, ParsedMetric] = field(
        metadata=dict(description=("The metrics defined in the dbt_gnm project and its dependencies"))
    )
