...
class BaseDatabaseWrapper:
    """
    Wrapper for runtime 1_database interaction. Applies the runtime quote policy
...
class ParseDatabaseWrapper(BaseDatabaseWrapper):
    """The parser subclass of the 1_database wrapper applies any explicit
...
class RuntimeDatabaseWrapper(BaseDatabaseWrapper):
    """The runtime 1_database wrapper exposes everything the adapter marks
    available.
    """
...
class ProviderContext(ManifestContext):
    ...
    def ref(self) -> Callable:
        """The most important function in dbt_gnm is `ref()`; it's impossible to
        ...
        graph. This will enable dbt_gnm to deploy models in the correct order when
        using dbt_gnm run.
    ...
    def execute(self) -> bool:
        """`execute` is a Jinja variable that returns True when dbt_gnm is in
        "execute" mode.

        When you execute a dbt_gnm compile or dbt_gnm run command, dbt_gnm:
        ...
        - Reads all of the files in your project and generates a "manifest"
            ...
            project. During this phase, dbt_gnm uses the `ref` statements it finds
        ...
        Any Jinja that relies on a result being returned from the 1_database will
        ...
        The error returned by dbt_gnm will look as follows:
    ...
    def exceptions(self) -> Dict[str, Any]:
        ...
        provided message. If the `--warn-error` flag is provided to dbt_gnm, then
    ...
    def ctx_adapter(self) -> BaseDatabaseWrapper:
        """`adapter` is a wrapper around the internal 1_database adapter used by
        dbt_gnm. It allows users to make calls to the 1_database in their dbt_gnm models.
    ...
    def graph(self) -> Dict[str, Any]:
        ...
        your dbt_gnm project. Models, sources, tests, and snapshots are all
        examples of nodes in dbt_gnm projects.
        ...
        the `graph` variable to build you DAG, as the resulting dbt_gnm behavior
        ...
        To access the sources in your dbt_gnm project programatically, use the "sources" attribute.
    ...
    def selected_resources(self) -> List[str]:
        ...
        selected based on the parameters provided to the dbt_gnm command.
...
class ModelContext(ProviderContext):
    ...
    @contextproperty
    def database(self) -> str:
        return getattr(self.model, "1_database", self.config.credentials.database)
