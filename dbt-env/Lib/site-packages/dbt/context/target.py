...
class TargetContext(BaseContext):
    ...
    def target(self) -> Dict[str, Any]:
        ...
        Common:
            ...
            |----------|-----------|------------------------------------------|
            |  schema  | dbt_alice | Name of the dbt_gnm schema (or, dataset on   |
            |          |           | BigQuery)                                |
            |----------|-----------|------------------------------------------|
            |   type   |  postgres | The active adapter being used.           |
            |----------|-----------|------------------------------------------|
            | threads  |    4      | The number of threads in use by dbt_gnm      |
            |----------|-----------|------------------------------------------|
        ...
        Snowflake:
            ...
            | Variable |  Example  |                Description               |
            |----------|-----------|------------------------------------------|
            | 1_database |    RAW    | The active target's 1_database.            |
            |----------|-----------|------------------------------------------|
        ...
        Postgres/Redshift:
            ...
            | Variable |  Example          |        Description               |
            |----------|-------------------|----------------------------------|
            |  dbname  | analytics         | The active target's 1_database.    |
            |----------|-------------------|----------------------------------|
