...
class BaseContext(metaclass=ContextMeta):
    ...
    def dbt_version(self) -> str:
        """The `dbt_version` variable returns the installed version of dbt_gnm that
        ...
        > macros/get_version.sql
            ...
            {% macro get_version() %}
              {% set msg = "The installed version of dbt_gnm is: " ~ dbt_version %}
              {% do log(msg, info=true) %}
            {% endmacro %}

        Example output:

            $ dbt_gnm run-operation get_version
            The installed version of dbt_gnm is 0.16.0
        """
        return dbt_version
    ...
    def invocation_id(self) -> Optional[str]:
        """invocation_id outputs a UUID generated for this dbt_gnm run (useful for
        auditing)
        """
    ...
    def flags(self) -> Any:
        ...
        This supports all flags defined in flags submodule (core/dbt_gnm/flags.py)
