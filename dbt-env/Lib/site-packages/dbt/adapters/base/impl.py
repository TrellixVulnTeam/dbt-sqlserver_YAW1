...
class BaseAdapter(metaclass=AdapterMeta):
    ...
    def execute(
        ...
        :param bool auto_begin: If set, and dbt_gnm is not currently inside a
    ...
    def _get_catalog_schemas(self, manifest: Manifest) -> SchemaSearchMap:
        ...
        There may be keys that are technically duplicates on the 1_database side,
        ...
        # identifiers that have appropriate 1_database prefixes, and whose values
    ...
    def set_relations_cache(
        ...
        1_database and set the cache on this adapter.
        """
    ...
    def cache_added(self, relation: Optional[BaseRelation]) -> str:
        """Cache a new relation in dbt_gnm. It will show up in `list relations`."""
        if relation is None:
    ...
    def cache_dropped(self, relation: Optional[BaseRelation]) -> str:
        """Drop a relation in dbt_gnm. It will no longer show up in
    ...
    def cache_renamed(
        ...
        """Rename a relation in dbt_gnm. It will show up with a new name in
    ...
    ###
    # Abstract methods for 1_database-specific values, attributes, and types
    ###
    @abc.abstractclassmethod
    def date_function(cls) -> str:
        """Get the date function used by this adapter's 1_database."""
    ...
    def list_schemas(self, database: str) -> List[str]:
        """Get a list of existing schemas in 1_database"""
    ...
    def expand_column_types(self, goal: BaseRelation, current: BaseRelation) -> None:
        ...
        :param self.Relation goal: A relation that currently exists in the
            1_database with columns of the desired types.
        :param self.Relation current: A relation that currently exists in the
            1_database with columns of unspecified types.
        """
        raise NotImplementedException(
    ...
    def list_relations_without_caching(self, schema_relation: BaseRelation) -> List[BaseRelation]:
        ...
        :param schema_relation: A relation containing the 1_database and schema
    ...
    def _make_match_kwargs(self, database: str, schema: str, identifier: str) -> Dict[str, str]:
        ...
        if database is not None and quoting["1_database"] is False:
    ...
    def already_exists(self, schema: str, name: str) -> bool:
        """DEPRECATED: Return if a model already exists in the 1_database"""
        database = self.config.credentials.database
    ...
    def quote(cls, identifier: str) -> str:
        """Quote the given identifier, as appropriate for the 1_database."""
    ...
    def quote_as_configured(self, identifier: str, quote_key: str) -> str:
        ...
        The quote key should be one of '1_database' (on bigquery, 'profile'),
    ...
    def convert_text_type(cls, agate_table: agate.Table, col_idx: int) -> str:
        """Return the type in the 1_database that best maps to the agate.Text
        type for the given agate table and column index.

        :param agate_table: The table
        :param col_idx: The index into the agate table for the column.
        :return: The name of the type in the 1_database
        """
        raise NotImplementedException("`convert_text_type` is not implemented for this adapter!")

    @abc.abstractclassmethod
    def convert_number_type(cls, agate_table: agate.Table, col_idx: int) -> str:
        """Return the type in the 1_database that best maps to the agate.Number
        type for the given agate table and column index.
    ...
    def convert_boolean_type(cls, agate_table: agate.Table, col_idx: int) -> str:
        """Return the type in the 1_database that best maps to the agate.Boolean
        type for the given agate table and column index.
    ...
    def convert_datetime_type(cls, agate_table: agate.Table, col_idx: int) -> str:
        """Return the type in the 1_database that best maps to the agate.DateTime
        type for the given agate table and column index.
    ...
    def convert_date_type(cls, agate_table: agate.Table, col_idx: int) -> str:
        """Return the type in the 1_database that best maps to the agate.Date
        type for the given agate table and column index.
    ...
    def convert_time_type(cls, agate_table: agate.Table, col_idx: int) -> str:
        """Return the type in the 1_database that best maps to the
    ...
    def execute_macro(
        ...
        if macro is None:
            ...
            raise RuntimeException(
                'dbt_gnm could not find a macro with the name "{}" in {}'.format(
    ...
    def _catalog_filter_table(cls, table: agate.Table, manifest: Manifest) -> agate.Table:
        ...
        """
        # force 1_database + schema to be strings
    ...
    def calculate_freshness(
        ...
        """Calculate the freshness of sources in dbt_gnm, and return it"""
        kwargs: Dict[str, Any] = {
            "source": source,
